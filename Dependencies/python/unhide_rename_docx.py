import os
import sys
import win32com.client as win32  # Import with alias "win32"

def unhide_text_in_word(file_path):
    """
    Opens a .doc or .docx file using Microsoft Word automation,
    and unhides all text by setting the Font.Hidden property to False
    on entire StoryRange objects rather than iterating word by word.
    """
    try:
        ...
        # Iterate over each story range in the document (main body, headers, footers, etc.)
        for story in doc.StoryRanges:
            current_range = story
            while current_range is not None:
                try:
                    # Directly unhide the entire range at once.
...
def sanitize_file_name(file_path):
    """
    Removes unwanted markers (like '_UNHIDE' or '(hidden)') from the filename,
    renames the file if needed, and returns the sanitized file path.
    """
...
def main():
    """
    Recursively processes all .doc and .docx files in the current directory:
      1) Renames files to remove unwanted markers.
      2) Unhides all hidden text by processing entire story ranges.
    """
