...
def update_xml_file(xml_path, lang_map):
    """Update PrintByLine tags in XML file - simply add/replace language suffix"""
    try:
        ...
        # This pattern ensures we only match complete PrintByLine tags
        ...
        for i, match in enumerate(matches):
            ...
            base_value = re.sub(r'_[A-Za-z]{2}$', '', original_text)
            
            # Add new suffix if needed
            new_value = base_value + (f"_{suffix}" if suffix else "")
            
            # FIXED: Replace only the content, not the entire tag
            # We're replacing just the text between the tags
